...
namespace Match3
{
	public class MatchPiece : MonoBehaviour
	{
		...
		public void SetupPiece(int row, int column, MatchTypes type, float time)
		{
			this.row = row;
			this.column = column;
			this.type = type;
		...
		public bool HasMatch(MatchType type)
		{
			switch (type)
			{
				case MatchType.LEFT:
		...
		public void SetMatch(MatchType type, bool value = true)
		{
			switch (type)
			{
				case MatchType.LEFT:
